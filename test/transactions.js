...
describe('Transactions', function() {
  ...
  describe('/rawtx/:txid', function() {
    it('should give the correct data', function(done) {
      ...
      var node = {
        getTransaction: sinon.stub().callsArgWith(1, null, helpthehomeless.Transaction().fromBuffer(new Buffer(hex, 'hex')))
      };
  ...
  describe('#transformInvTransaction', function() {
    it('should give the correct data', function() {
      ...
      var tx = helpthehomeless.Transaction().fromBuffer(new Buffer(rawTx, 'hex'));

      var node = {
        network: helpthehomeless.Networks.livenet
      };
    ...
    it('should detect RBF txs', function() {
      ...
      _.each(testCases, function(tc) {
        var tx = helpthehomeless.Transaction().fromBuffer(new Buffer(tc.rawTx, 'hex'));
        var result = transactions.transformInvTransaction(tx);
